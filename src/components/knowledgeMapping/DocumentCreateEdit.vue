    ...
    <div v-if="localDocument"
        ...
        <div class="w-full">
            ...
            <div class="py-2">
                ...
                <textarea v-model="localDocument.description.en" class="form-input rounded-md shadow-sm mt-1 block w-full"
                    :placeholder="L_('Description') + ' (EN)'" @input="onLocalDocumentChange"></textarea>
                <textarea v-model="localDocument.description.fr" class="form-input rounded-md shadow-sm mt-1 block w-full"
                    :placeholder="L_('Description') + ' (FR)'" @input="onLocalDocumentChange"></textarea>
            </div>
            ...
            <div class="py-2">
                <label class="font-bold">{{ L_("Original File Information") }}</label>
                <div>
                    <div>
                        <label>{{ L_("Name") }}</label>
                        <input v-model="localDocument.original.name"
                            class="form-input rounded-md shadow-sm mt-1 block w-full bg-gray-100 text-gray-400 cursor-not-allowed dark:bg-gray-700 dark:text-gray-200"
                            disabled />

                        <label>{{ L_("Type") }}</label>
                        <input v-model="localDocument.original.type"
                            class="form-input rounded-md shadow-sm mt-1 block w-full bg-gray-100 text-gray-400 cursor-not-allowed dark:bg-gray-700 dark:text-gray-200"
                            disabled />

                        <label>{{ L_("Size") }}</label>
                        <input v-model="localDocument.original.size"
                            class="form-input rounded-md shadow-sm mt-1 block w-full bg-gray-100 text-gray-400 cursor-not-allowed dark:bg-gray-700 dark:text-gray-200"
                            disabled />

                        <label>{{ L_("Last Modified") }}</label>
                        <input v-model="localDocument.original.lastModified"
                            class="form-input rounded-md shadow-sm mt-1 block w-full bg-gray-100 text-gray-400 cursor-not-allowed dark:bg-gray-700 dark:text-gray-200"
                            disabled />
                    </div>
                </div>
